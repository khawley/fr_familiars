...
class Chests(FrBase):
    ...
    def print_chest_results(self):
        ...
        sorted_item_map = sorted([self.item_map[item_id]
                                 ...
                                 key=lambda x: x["type"])
        self.echo_n("Chest Results")
        # print treasure + gems separately, since they aren't in the item_map
        self.echo_n("- Treasure: {}".format(self.item_totals["treasure"]))
        self.echo_n("- Gems: {}".format(self.item_totals.get("gem", "0")))
        self.echo_n()
